...
namespace Net
{
	static const int MAX_OUTBOUND_CONNECTIONS = GetArg("-maxoutbound", 16);
	...
	CNode* ConnectNode(CAddress addrConnect, int64 nTimeout)
	{
		...
		printf("trying connection %s lastseen=%.1fhrs\n",
			...
			(double)(addrConnect.nTime - GetUnifiedTimestamp())/3600.0);
		...
		if (ConnectSocket(addrConnect, hSocket))
		{
			...
			pnode->nTimeConnected = GetUnifiedTimestamp();
			return pnode;
		}
		else
		{
					...
					if (lockSend)
					{
						...
						if (!vSend.empty())
						{
							...
							if (nBytes > 0)
							{
								...
								pnode->nLastSend = GetUnifiedTimestamp();
							}
							else if (nBytes < 0)
							{
								// error
	...
	// testnet dns seed begins with 't', all else are Nexus dns seeds.
	...
	void ThreadDNSAddressSeed2(void* parg)
	{
		...
		if (true /*!fTestNet*/)  // Nexus enables dns seeding with testnet too
		{
			...
			for (unsigned int seed_idx = 0; seed_idx < ARRAYLEN(strDNSSeed); seed_idx++) {
				...
				if (LookupHost(strDNSSeed[seed_idx][1], vaddr))
				{
					BOOST_FOREACH(CNetAddr& ip, vaddr)
					{
						...
						addr.nTime = GetUnifiedTimestamp() - 3*nOneDay - GetRand(4*nOneDay); // use a random age between 3 and 7 days old
						vAdd.push_back(addr);
						found++;
						
						addrman.Add(addr, ip, true);
					}
				}
			}
		}
	...
	void ThreadOpenConnections2(void* parg)
	{
		...
		loop
		{
			...
			loop
			{
				...
				if (!addr.IsIPv4() || !addr.IsValid() || addr == addrLocalHost) // || setConnected.count(addr.GetGroup()) )
					break;
		...
		while (!fShutdown)
		{
			...
			BOOST_FOREACH(CNode* pnode, vNodesCopy)
			{
				...
				// Send messages
				{
					TRY_LOCK(pnode->cs_vSend, lockSend);
					if (lockSend)
						Core::SendMessages(pnode, pnode == pnodeTrickle);
				}
				if (fShutdown)
					return;
			}
	...
	bool BindListenPort(string& strError)
	{
		...
		if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
		{
			...
			if (nErr == WSAEADDRINUSE)
				strError = strprintf(_("Unable to bind to port %d on this computer.  Nexus is probably already running."), ntohs(sockaddr.sin_port));
			else
	...
	bool StopNode()
	{
		...
		do
		{
			...
			if (nThreadsRunning == 0)
				break;
			if (GetUnifiedTimestamp() - nStart > 20)
				break;
			Sleep(20);
		} while(true);
		...
		if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadNexusMiner still running\n");
